...
if(!empty($_POST)){
    ...
    if(empty($_POST['pseudo']) || !preg_match("#^[a-zA-z0-9_]+$#", $_POST['pseudo'])){
        $errors['pseudo']= $_SESSION['flash']['danger']= 'Votre pseudo n\'est pas  valide';
        //var_dump($errors['pseudo']);
    }else{
        // ici nous ne voulons pas à ce que deux pseudo du meme type soit soumis
       $query = "SELECT * FROM `admin` WHERE pseudo = ?";
       $sql= $db->prepare($query);
       $sql->execute([$_POST['pseudo']]);

       if($sql->fetch()){
        $errors['pseudo']=$_SESSION['flash']['danger'] = 'ce pseudo est déjà pris';
       }

    }
     //ici on gere les erreurs pour le champs email
     if(empty($_POST['email']) || !filter_var($_POST['email'],FILTER_VALIDATE_EMAIL)){
        $errors['email'] = $_SESSION['flash']['danger'] = 'Votre email n\'est pas  valide';
       ...
       if($sql->fetch()){
        $errors['email']= $_SESSION['flash']['danger']  = 'ce email est déjà pris';
       }
    }

     //ici on gere les erreurs pour le champs password
     if(empty($_POST['mdp']) || $_POST['mdp'] !== $_POST['mdp_confirm']){
        $errors['mdp'] = $_SESSION['flash']['danger']= 'Vous devez rentrez un mot de pass valide et confirmé ';
        
    }

       if(empty($errors)){
        $query= ("INSERT INTO `admin` (pseudo,email,mdp,confirmation_token) VALUES (?,?,?,?) ");
        $sql = $db->prepare($query);
