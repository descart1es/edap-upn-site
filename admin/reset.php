...
$query= "SELECT * FROM `admin` WHERE id = ? AND reset_token = ? AND reset_at > DATE_SUB(NOW(), INTERVAL 30 MINUTE) AND confirmed_at IS NOT NULL";
$req=$db->prepare($query);
$req->execute([$adminId, $token]);
$admin = $req->fetch();
...
if($admin){
    if(!empty($_POST)){
            ...
            if(!empty($_POST['mdp']) && $_POST['mdp'] == $_POST['mdp_confirm']){
             ...
             $query= " UPDATE `admin` SET mdp = ?, reset_token = NULL, reset_at = NULL WHERE id = ? ";
            
            //$db->prepare($query)->execute;
             $db->prepare($query)->execute([$mdp,$adminId]);
